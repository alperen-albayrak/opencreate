# Rendering: the Vibrant-Visuals-Parity Roadmap

The plan to reach **Vibrant Visuals (VV) feature parity or better** with a
modular, physically-grounded renderer. This page **supersedes and expands** the
"Graphics roadmap — vibrant visuals" section in [../roadmap.md](../roadmap.md):
...
(later) many dynamic lights. This is the VV path and unlocks its payoff:
...
- **Transparency stays forward** (water, cutout leaves, sky, clouds), composited
  after the lighting resolve — exactly as VV does.
...
## Natural phenomena (modelled from nature, ≥ VV)
...
(scotopic blue-shift) VV doesn't fully do. Stars: **apparent magnitude →
...
- **Stage 1 — foundations:** the data-driven **block registry** (the keystone —
  see [matter model](matter-model.md)) → `physical.rs` → Scene UBO → the
  **deferred scaffold + RGB light-field bake** → the data registries
  (`FluidDef`/`GasDef`/`EnvDef`). The big rewrite, done once.
- **Stage 2 — lighting, built once on deferred:** RGB-light consumption;
  directional **sun/moon + shadows** (revive the shelved cascades, per-dimension
  via `EnvDef`); **atmosphere & volumetrics**; stars by magnitude + spectral
  color.
- **Stage 3 — materials:** **Cook–Torrance + Fresnel–Schlick** PBR; per-texel
  **normal + MER(S)**; leaf/grass SSS; **many clustered dynamic lights**.
- **Stage 4 — beyond-VV + perf:** IBL reflections, per-biome color grading,
